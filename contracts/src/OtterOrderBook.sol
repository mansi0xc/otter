...
/// @notice Collects signed orders into per-pool batch windows. Holds no funds and
///         executes nothing.
///
...
contract OtterOrderBook {
    ...
    /// @notice Submit signed orders into the open batch. Anyone may relay.
    /// @dev Every order must target the same pool, which is what lets one
    ///      rollover check cover the whole array.
